// <copyright file="OkOrElseTaskExtension.cs" company="Markus - Iorio">
...
/// <see cref="Result{T, E}"/> values with a lazily-computed error using <see cref="Task{TResult}"/>.
/// </summary>
public static class OkOrElseTaskExtension
{
        ...
        where T : notnull
    {
        ...
        /// <exception cref="ArgumentNullException">Thrown when <paramref name="self"/> or <paramref name="error"/> is <see langword="null"/>.</exception>
        public async Task<Result<T, E>> OkOrElseAsync<E>(Func<E> error)
            where E : notnull
        {
        ...
        /// <returns>A task producing the corresponding <see cref="Result{T, E}"/>.</returns>
        /// <exception cref="ArgumentNullException">Thrown when <paramref name="self"/> or <paramref name="error"/> is <see langword="null"/>.</exception>
        public async Task<Result<T, E>> OkOrElseAsync<E>(Func<Task<E>> error)
            where E : notnull
        {
            ArgumentNullException.ThrowIfNull(self);
            ArgumentNullException.ThrowIfNull(error);
