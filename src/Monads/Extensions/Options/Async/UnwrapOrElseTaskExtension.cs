...
/// <summary>
/// Provides asynchronous extension members for unwrapping <see cref="Option{T}"/> values with a lazy fallback using <see cref="Task{TResult}"/>.
/// </summary>
public static class UnwrapOrElseTaskExtension
{
    ...
    {
        /// <summary>
        /// Awaits the option task and returns its wrapped value when Some; otherwise returns the result of the synchronous fallback.
        /// </summary>
        /// <param name="fallback">The synchronous function invoked when the awaited option is None.</param>
        /// <returns>A task producing the wrapped value when Some; otherwise <c>fallback()</c>.</returns>
        ...
        public async Task<T> UnwrapOrElseAsync(Func<T> fallback)
        {
        ...
        /// <summary>
        /// Awaits the option task and returns its wrapped value when Some; otherwise returns the result of the asynchronous fallback.
        /// </summary>
        /// <param name="fallback">The asynchronous function invoked when the awaited option is None.</param>
        /// <returns>A task producing the wrapped value when Some; otherwise <c>fallback()</c>.</returns>
        /// <exception cref="ArgumentNullException">Thrown when <paramref name="self"/> or <paramref name="fallback"/> is <see langword="null"/>.</exception>
        public async Task<T> UnwrapOrElseAsync(Func<Task<T>> fallback)
        {
            ArgumentNullException.ThrowIfNull(self);
            ArgumentNullException.ThrowIfNull(fallback);
