// <copyright file="MapValueTaskExtension.cs" company="Markus - Iorio">
...
/// <summary>
/// Provides asynchronous extension members for mapping the wrapped value of <see cref="Option{T}"/> instances using <see cref="ValueTask{TResult}"/>.
/// </summary>
public static class MapValueTaskExtension
{
    extension<T>(ValueTask<Option<T>> self)
        where T : notnull
    {
        /// <summary>
        /// Awaits the option value task and transforms its Some value using a synchronous mapping function.
        /// </summary>
        ...
        /// <returns>A value task producing Some of the mapped value when Some; otherwise None.</returns>
        /// <exception cref="ArgumentNullException">Thrown when <paramref name="operation"/> is <see langword="null"/>.</exception>
        public async ValueTask<Option<U>> MapAsync<U>(Func<T, U> operation)
            where U : notnull
        {
        ...
        /// <summary>
        /// Awaits the option value task and transforms its Some value using an asynchronous mapping function.
        /// </summary>
        /// <typeparam name="U">The result type of the mapped value.</typeparam>
        /// <param name="operation">The asynchronous function applied to the wrapped value when Some.</param>
        /// <returns>A value task producing Some of the mapped value when Some; otherwise None.</returns>
        /// <exception cref="ArgumentNullException">Thrown when <paramref name="operation"/> is <see langword="null"/>.</exception>
        public async ValueTask<Option<U>> MapAsync<U>(Func<T, ValueTask<U>> operation)
            where U : notnull
        {
            ArgumentNullException.ThrowIfNull(operation);
