...
/// <summary>
/// Provides asynchronous extension members for binding operations on <see cref="Result{T, E}"/> instances using <see cref="Task{TResult}"/>.
/// </summary>
public static class BindTaskExtension
{
        ...
        where E : notnull
    {
        ...
        public async Task<Result<U, E>> BindAsync<U>(Func<T, Result<U, E>> operation)
            where U : notnull
        {
        ...
        /// <exception cref="InvalidOperationException">Thrown if the operation returns null.</exception>
        /// <exception cref="UnreachableException">Thrown if the result is neither <see cref="Ok{T, E}"/> nor <see cref="Err{T, E}"/>.</exception>
        public async Task<Result<U, E>> BindAsync<U>(Func<T, Task<Result<U, E>>> operation)
            where U : notnull
        {
            ArgumentNullException.ThrowIfNull(operation);

            return await self.MatchAsync(operation, err => Task.FromResult(Result.Err<U, E>(err)))
