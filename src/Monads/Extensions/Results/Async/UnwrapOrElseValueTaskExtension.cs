...
/// <summary>
/// Provides asynchronous extension members for unwrapping <see cref="Result{T, E}"/> values with a lazy fallback computed from the error, using <see cref="ValueTask{TResult}"/>.
/// </summary>
public static class UnwrapOrElseValueTaskExtension
{
    ...
    {
        /// <summary>
        /// Awaits the result value task and returns the Ok value; otherwise returns the synchronous fallback applied to the error.
        /// </summary>
        /// <param name="fallback">The synchronous function invoked with the error when the awaited result is Err.</param>
        /// <returns>A value task producing the Ok value when Ok; otherwise <c>fallback(error)</c>.</returns>
        /// <exception cref="ArgumentNullException">Thrown when <paramref name="fallback"/> is <see langword="null"/>.</exception>
        public async ValueTask<T> UnwrapOrElseAsync(Func<E, T> fallback)
        {
        ...
        /// <summary>
        /// Awaits the result value task and returns the Ok value; otherwise returns the asynchronous fallback applied to the error.
        /// </summary>
        /// <param name="fallback">The asynchronous function invoked with the error when the awaited result is Err.</param>
        /// <returns>A value task producing the Ok value when Ok; otherwise <c>fallback(error)</c>.</returns>
        /// <exception cref="ArgumentNullException">Thrown when <paramref name="fallback"/> is <see langword="null"/>.</exception>
        public async ValueTask<T> UnwrapOrElseAsync(Func<E, ValueTask<T>> fallback)
        {
            ArgumentNullException.ThrowIfNull(fallback);

            return await self.MatchAsync(ValueTask.FromResult, fallback).ConfigureAwait(false);
        }
    }
