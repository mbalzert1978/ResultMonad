...
/// <summary>
/// Provides asynchronous extension methods for error recovery operations on <see cref="Result{T, E}"/> instances using <see cref="Task{TResult}"/>.
/// </summary>
public static class OrElseTaskExtension
{
    ...
    public static async Task<Result<T, F>> OrElseAsync<T, E, F>(
    ...
    public static async Task<Result<T, F>> OrElseAsync<T, E, F>(
        ...
        Func<E, Task<Result<T, F>>> operation
    )
        where T : notnull
        where E : notnull
        where F : notnull
