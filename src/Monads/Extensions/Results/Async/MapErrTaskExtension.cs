...
/// <summary>
/// Provides asynchronous extension members for mapping error values within <see cref="Result{T, E}"/> instances using <see cref="Task{TResult}"/>.
/// </summary>
public static class MapErrTaskExtension
{
        ...
        where E : notnull
    {
        ...
        public async Task<Result<T, F>> MapErrAsync<F>(Func<E, F> operation)
            where F : notnull
        {
        ...
        /// <exception cref="InvalidOperationException">Thrown if the operation returns null.</exception>
        /// <exception cref="UnreachableException">Thrown if the result is neither <see cref="Ok{T, E}"/> nor <see cref="Err{T, E}"/>.</exception>
        public async Task<Result<T, F>> MapErrAsync<F>(Func<E, Task<F>> operation)
            where F : notnull
        {
            ArgumentNullException.ThrowIfNull(self);
            ArgumentNullException.ThrowIfNull(operation);
